...
function encryptValue(value: string, hashedPassword: string): string {
  ...
  const cipher = crypto.createCipheriv('aes-256-gcm', Buffer.from(hashedPassword.slice(2), 'hex'), iv);
...
function decryptValue(encryptedValue: string, hashedPassword: string): string {
  ...
  const decipher = crypto.createDecipheriv('aes-256-gcm', Buffer.from(hashedPassword.slice(2), 'hex'), iv);
...
function setupUserCredentials(password: string) {
  ...
  const encryptedInternalNullifier = encryptValue(internalNullifier, hashedPassword);
  const encryptedIdentitySecret = encryptValue(identitySecret, hashedPassword);
...
export {setupUserCredentials, hashPassword, generateRandomValue, generateIdentityCommitment, encryptValue, decryptValue, setAuthPassword};
