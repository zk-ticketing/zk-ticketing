...
const PasswordVerification: React.FC<PasswordVerificationProps> = ({ 
  ...
  const handleVerifyAndCachePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const decryptedInternalNullifier = decryptValue(encryptedInternalNullifier, hashedPassword);
    ...
    <Form onSubmit={handleVerifyAndCachePassword}>
      ...
      <PasswordInput>
        ...
        <VisibilityToggle onClick={togglePasswordVisibility}>
          <Image
            src={passwordVisible ? "/hidden-icon.svg" : "/visible-icon.svg"}
            alt={passwordVisible ? "hide password" : "reveal password"}
            width={20}
            height={20}
          />
        </VisibilityToggle>
